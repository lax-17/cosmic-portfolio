...
const CosmicSoundEffects: React.FC = () => {
  ...
  return (
    <>
      {/* Simplified Sound Control Toggle */}
      <motion.button
        className={`fixed bottom-32 right-6 z-50 p-2 rounded-lg shadow-lg transition-all duration-300 ${
          isEnabled
      ...
      {isEnabled && (
        <motion.div
          className="fixed bottom-40 right-6 z-50 px-2 py-1 bg-background/90 backdrop-blur-sm border border-border rounded text-xs text-muted-foreground pointer-events-none"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
